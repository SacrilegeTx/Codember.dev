...
const isAlphanumeric = str =>
  str.match(new RegExp(REGEX_ALPHANUMERIC, 'g')) !== null
...
const file = await fs
  .readFile('files\\database_attacked.txt', 'utf-8')
...
file.split('\r\n').forEach(row => {
  ...
  if (username && !isAlphanumeric(username) && !inArray(invalidUsers, username))
    invalidUsers.push(username)
  if (!isValidEmail(email) && !inArray(invalidUsers, username))
    invalidUsers.push(username)
  if (age && !isNumeric(age) && !inArray(invalidUsers, username))
    invalidUsers.push(username)
  if (location && !isString(location) && !inArray(invalidUsers, username))
    invalidUsers.push(username)
})
